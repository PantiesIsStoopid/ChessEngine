...
def parse_board(file_path):
    ...
    for i in range(8):  # Only consider the 8 lines corresponding to the board
        ...
        if len(row) != 8:
            print(
                f"Warning: Invalid row format at line {i+1}. Expected 8 pieces, got {len(row)}."
            )
            continue
...
def render_board_gui(board, board_frame):
    ...
    for rank in range(8):  # Display rank from 1 to 8 (with white at the bottom)
        for file in range(8):
            piece = board[7 - rank][
                file
            ]  # Reverse the rank order to ensure white at the bottom
            ...
            if img_file:
                ...
                label = tk.Label(
                    board_frame, text="", width=4, height=2, font=("Arial", 18)
                )
...
def monitor_file_for_changes(file_path, board_frame, update_callback):
    ...
    while True:
        ...
        if current_modified_time != last_modified_time:
            ...
            update_callback(
                file_path, board_frame
            )  # Trigger the callback to update the board
...
def create_gui(file_path):
    ...
    monitor_thread = threading.Thread(
        ...
        args=(file_path, board_frame, render_chess_board_from_file),
    )
